...
import org.nemesis.editor.impl.Hold;
...
import static org.nemesis.editor.ops.DocumentPreAndPostProcessor.NO_OP;
...
public final class DocumentOperator {
    ...
    private static void eqRun(DocumentRunnable r) throws BadLocationException {
     ...
     * @param run A runnable
     */
    public void runOp(StyledDocument doc, DocumentRunnable run) {
...
//                });
    ...
    public static <T, E extends Exception> T render(Document doc, DocumentSupplier<T, E> supp) throws BadLocationException, E {
    ...
    private static final ThreadLocal<Set<Document>> LOCKED_DOCS = ThreadLocal.withInitial(HashSet::new);
    public static <T, E extends Exception> T runAtomic(StyledDocument doc, DocumentSupplier<T, E> supp) throws BadLocationException, E {
        ...
        Hold<T, E> hold = new Hold<>();
        Set<Document> locked = LOCKED_DOCS.get();
        try {
            locked.add(doc);
    ...
    public static <T, E extends Exception> T runAtomicAsUser(StyledDocument doc, DocumentSupplier<T, E> supp) throws BadLocationException, E {
        // If we are already locked, don't attempt it twice - we may wind up acquiring
        // locks out-of-order
        if (isAtomicLocked(doc)) {
            return supp.get();
        ...
        Hold<T, E> hold = new Hold<>();
        // Since there is no equivalent test for NbDocument.runAtomic, we
        // do our own bookkeeping.  In practice, we are almost always dealing
        // with NbEditorDocument, so the BaseDocument call will work
        Set<Document> locked = LOCKED_DOCS.get();
        try {
            locked.add(doc);
    ...
    public static <T, E extends Exception> T runWriteLocked(StyledDocument doc, DocumentSupplier<T, E> supp) throws BadLocationException, E {
        if (!(doc instanceof BaseDocument)) {
            return supp.get();
        }
    ...
    public static boolean isAtomicLocked(Document doc) {
        return (doc instanceof BaseDocument && ((BaseDocument) doc).isAtomicLock())
                || LOCKED_DOCS.get().contains(doc);
    }
