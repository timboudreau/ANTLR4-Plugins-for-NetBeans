...
public class ReplaceRanges extends SimpleRefactoringElementImplementation {
    ...
    public static void create(ExtractionKey<?> key, FileObject fo, List<IntRange<? extends IntRange>> ranges, String oldText, String newText, Consumer<RefactoringElementImplementation> c) throws IOException, BadLocationException {
        ...
        for (int i = 0; i < ranges.size(); i++) {
            ...
            bounds.add(bds);
            children.add(new RangeChild(key, range, bds, fo, origFileText, newText, disabled, performed));
        }
    ...
    @NbBundle.Messages(value = {
        ...
        "# {1} - originalName",
        "# {2} - newName",
        "replaceTitle=Replace {0} occurrences of ''{1}'' with ''{2}''"})
    @Override
    public String getText() {
        return Bundle.replaceTitle(bounds.size(), oldText, newText);
    }

    @NbBundle.Messages(value = {"# {0} - the kind of element",
        "replaceName=Replace name in {0}"})
    @Override
    public String getDisplayText() {
        return Bundle.replaceName(key.name()); // XXX pass this in
    }
    ...
    static final class RangeChild extends SimpleRefactoringElementImplementation {
        ...
        @Messages({
            ...
            "# {0} - start",
            "# {1} - end",
            "invalidChange=Invalidated change at {0}:{1}"
        })
        public String getText() {
            try {
                ...
                dead = true;
                return Bundle.invalidChange(range.start(), range.end());
            }
        }
        ...
        public String getDisplayText() {
            try {
                return Bundle.oneChangeText(bounds.getBegin().getLine(), bounds.getBegin().getColumn());
