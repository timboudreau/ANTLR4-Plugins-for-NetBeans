    ...
    protected Problem doPrepare(RefactoringElementsBag bag) {
        return inParsingContext(() -> {
            ...
            Problem p = uf.findUsages(this::isCancelled, file, extraction, toRename, extraction.namedRegions(key),
                    (IntRange<? extends IntRange> target, String regionText, FileObject targetFile, String typeName, Extraction sourceExtraction) -> {
            ...
            /*
            List<OneRangeInOneFileChange> all = new ArrayList<>();
            Set<ModificationResult> results = new LinkedHashSet<>(all.size());
            Problem p = uf.findUsages(this::isCancelled, file, extraction, toRename, extraction.namedRegions(key),
                    (IntRange<? extends IntRange> target, String regionText, FileObject targetFile, String typeName, Extraction sourceExtraction) -> {
                        OneRangeInOneFileChange change = new OneRangeInOneFileChange(target, regionText, refactoring, targetFile,
                                typeName, target);
                        bag.add(refactoring, change);
                        System.out.println("size " + all.size() + "before change " + change);
                        all.add(change);
                        System.out.println("size now " + all.size());
                        try {
                            System.out.println(" before add mods " + results.size());
                            results.add(change.toModificationResult());
                            System.out.println(" after add modes " + results.size());
