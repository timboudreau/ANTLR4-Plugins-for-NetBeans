...
mod :
    doc=doc_comment? outer_attribute* Pub?
        Mod name=Ident (LeftBrace (mod |
        use_statement | function)*
        RightBrace)?
    ;
...
variable_pattern :
                 LeftParen (variable_spec
                     (Comma variable_spec)*
                     Comma?) RightParen
                 ;

variable_spec :
              (name=variable_name (Colon
                  type=type_spec)?)
              | anon=Underscore
              ;
...
match :
      Match deref=Asterisk? var=expression
          match_block
      ;

match_block :
            LeftBrace (first=match_case (Comma
                more=match_case)*)?
                defaultCase=default_match_case?
                Comma? RightBrace
            ;
...
type_hint :
          LeftAngleBracket type_spec As
              type_spec RightAngleBracket
          ;
...
float_literal :
              value=FloatLiteral type=float_subtype?
                  cast=float_cast
              ;
...
function_spec :
              doc=doc_comment? attrs=outer_attribute*
                  vis=visibility? Fn name=function_name
                  lifetimes=lifetime_spec?
                  LeftParen params=parameter_list?
                  RightParen return_type?
              ;
...
extern_block :
             attrs=outer_attribute*
                 Extern LeftBrace (extern_function_statement
                 (Comma
...
closure :
        (attrs=outer_attribute* lifetimes=lifetime_spec?
            mv=Move? Pipe params=parameter_list?
            Pipe return_type? (body=block
            | expr=expression))
        | (attrs=outer_attribute* mv=Move?
            DoublePipe return_type? (body=block
            | expr=expression))
        | (attrs=outer_attribute* mv=Move?
            Pipe Pipe return_type? (body=block
            | expr=expression))
        ;

struct :
       (doc=doc_comment? outer_attribute*
           Struct name=Ident lifetimes=lifetime_spec?
           LeftBrace (struct_item (Comma
           struct_item)* Comma?)?
...
// XXX could split this out and include it in statement_body - checking for Semicolon here may cause problems
       | (doc=doc_comment?
           outer_attribute* Struct name=Ident
           lifetimes=lifetime_spec?
           LeftParen (type_spec (Comma
           type_spec)* Comma?)?
...
struct_item :
            (doc=doc_comment?
                outer_attribute*
                inner_attribute* name=Ident
                Colon props=param_props
                type=type_spec)
            ;

struct_instantiation :
                     type=type_spec
                         LeftBrace (struct_instantiation_item
                         (Comma
...
lifetime_spec :
              LeftAngleBracket first=lifetime
                  (Comma more=lifetime)*
                  RightAngleBracket
              ;
...
parameter_list :
               (parameter_spec (Comma
                   parameter_spec)*)
               | (selfref=Ampersand? self=SelfRef
                   (Comma parameter_spec)*)
               ;

parameter_spec :
               props=param_props name=variable_name
                   (Colon Ampersand? life=lifetime?
                   type=type_spec)?
               ;

param_props :
            ref=Ampersand? life=lifetime?
                mutable=Mut? dynamic=Dyn?
            ;
...
type_pattern :
             LeftParen type_spec (Comma
                 type_spec) RightParen
             ;

type_spec :
          path=path_head? type=Ident {
 referencedTypes.add(_ctx.getText()); } #Named
...
metaitem :
         ...
         | id=Ident LeftParen literal
             RightParen
         | id=Ident LeftParen metaitem (Comma
             metaitem) RightParen
         ;
...
MatchRangeInclusive :
                    DEC_DIGITS TripleDot
                        DEC_DIGITS
                    ;

RangeInclusive :
               DEC_DIGITS DotDotEquals
                   DEC_DIGITS
               ;
