...
OpenNumberedList :
                 INLINE_WHITESPACE+ DIGIT+
                     DOT { updateIndentDepth(); }
                     -> more, pushMode(ORDERED_LIST);

OpenBlockQuote :
               INLINE_WHITESPACE* GT { clearIndentDepth(); }
                   -> pushMode(BLOCKQUOTE);

OpenPreformattedText :
                     BACKTICK BACKTICK
                         BACKTICK -> pushMode(PREFORMATTED);
...
OpenHr :
       ((DASH DASH DASH+) | (ASTERISK
           ASTERISK ASTERISK+) | (DASH
           SPACE DASH SPACE (DASH SPACE)+)
           | (ASTERISK SPACE ASTERISK
           SPACE (ASTERISK SPACE)+ SPACE*)) { clearIndentDepth(); }
           -> more, mode(HR);
...
ParaWords :
          (WORD_LIKE SAFE_PUNCTUATION? (NEWLINE*?
              INLINE_WHITESPACE+
              WORD_LIKE SAFE_PUNCTUATION?)*)
          | (WORD_LIKE SAFE_PUNCTUATION?
              (INLINE_WHITESPACE*?
              WORD_LIKE SAFE_PUNCTUATION?)*)
          | (WORD_LIKE SAFE_PUNCTUATION?
...
NestedOrderedListPrologue :
                          ((DOUBLE_NEWLINE?
                              ...
                              INLINE_WHITESPACE+
                              DIGIT DOT
                              INLINE_WHITESPACE) { updateIndentDepth() }?
                          ;
...
ReturningOrderedListPrologue :
                             ((DOUBLE_NEWLINE?
                                 ...
                                 DIGIT
                                 DOT
                                 INLINE_WHITESPACE) { lastIndentChangeWasNegative() }?
                             ;
...
CloseOrderedListItem :
                     DOUBLE_NEWLINE { clearIndentDepth(); }
                         -> popMode;
