...
variable_binding : ( Let props=variable_props name=variable_spec ) #UnassignedBinding
        ...
        aprops=assignee_props name=variable_name )| exp=expression )) #DestructuringLetBinding;
...
match_case : ( first=literal ( Pipe more=literal )* FatArrow ( statement_body |
        ...
        | ( range=MatchRangeInclusive FatArrow ( statement_body | expression |
        ...
        FatArrow ( statement_body | expression | block )) #ExpressionCase
...
expression : literal cast=variable_cast? #LiteralExpression
        ...
        | Bang? LeftParen ls=expression comparison_operator rs=expression
...
type_spec : path=path_head? type=Ident { referencedTypes.add(_ctx.getText()); } #Named
