...
public class CodeCompletionCoreOrig {
    ...
    static final class NoNullsMap<T, R> extends HashMap<T, R> {
        ...
        public R put(T key, R value) {
            assert value != null : "Putting ull value for " + key;
    ...
    public static class CandidatesCollection {
         ...
         * subsequent tokens
         */
        public Map<Integer, List<Integer>> tokens = new NoNullsMap<>();
        /**
         * Collection of Rule candidates, each with the callstack of rules to
         * reach the candidate
         */
        public Map<Integer, List<Integer>> rules = new NoNullsMap<>();
        /**
    ...
    public CodeCompletionCoreOrig(Parser parser, Set<Integer> preferredRules, Set<Integer> ignoredTokens) {
        this.parser = parser;
        this.atn = parser.getATN();
        this.vocabulary = parser.getVocabulary();
        this.ruleNames = parser.getRuleNames();
        if (preferredRules != null) {
            this.preferredRules = new TreeSet<>(preferredRules);
        }
        if (ignoredTokens != null) {
            this.ignoredTokens = new TreeSet<>(ignoredTokens);
        }
    }
    ...
    public CandidatesCollection collectCandidates(int caretTokenIndex, ParserRuleContext context) {
        ...
        tokenStream.seek(currentIndex);
        CCLog.log(this, "Start", caretTokenIndex, (tokens.isEmpty() ? "emptytokens" : tokens.get(tokens.size() - 1)), " toks ", tokens.size());
        ...
        if (this.showResult && logger.isLoggable(Level.FINE)) {
            ...
            for (Map.Entry<Integer, List<Integer>> entry : this.candidates.tokens.entrySet()) {
                ...
                assert entry.getValue() != null : "Null value in map for " + entry.getKey();
    ...
    private void collectFollowSets(ATNState s, ATNState stopState, LinkedList<FollowSetWithPath> followSets,
        ...
        if (seen.contains(s)) {
            CCLog.log(this, "seen", s, stopState, " rs", ruleStack);
            return;
        }
