...
class AntlrCompletionItem implements CompletionItem {
    ...
    public int getPreferredWidth(Graphics g, Font defaultFont) {
        double width = HtmlRenderer.renderString(text, g, 5, 0,
                2000, 200, font, Color.BLACK, HtmlRenderer.STYLE_CLIP, false) + 10;
    ...
    public void render(Graphics g, Font defaultFont, Color defaultColor, Color backgroundColor, int width, int height, boolean selected) {
        ...
        int baseline = g.getFontMetrics(font).getMaxAscent();
        String html = "<font name=\"" + font.getFamily() + "\">";
        HtmlRenderer.renderHTML(html + text, g, 5, baseline, width, height, font, color,
