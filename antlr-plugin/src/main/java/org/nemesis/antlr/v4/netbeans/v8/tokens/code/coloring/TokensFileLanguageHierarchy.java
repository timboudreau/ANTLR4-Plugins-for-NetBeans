 ...
 */
public class TokensFileLanguageHierarchy extends LanguageHierarchy<ANTLRv4TokenId> {
    ...
    private static final Language<ANTLRv4TokenId>     language
                                 = new TokensFileLanguageHierarchy().language();

    private static void init() {
        TokensFileTokenFileReader reader = new TokensFileTokenFileReader();
    ...
    protected synchronized Lexer<ANTLRv4TokenId> createLexer(LexerRestartInfo<ANTLRv4TokenId> info) {
        return new TokensFileLexerAdaptor(info);
    }

    @Override
    protected String mimeType() {
        return "text/x-tokens";
    }
}