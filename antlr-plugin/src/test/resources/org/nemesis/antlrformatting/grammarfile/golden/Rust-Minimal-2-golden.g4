...
items : ( use_statement | extern_import_statement
          | extern_block | inner_attribute
          | unsafe_block | struct
          | function | mod )*;

mod : doc=doc_comment?
         outer_attribute* Pub? Mod
         name=Ident ( LeftBrace (
         mod | use_statement |
         function )* RightBrace )?;

extern_import_statement : extern_import
                             Semicolon;

use_statement : outer_attribute*
...
block : LeftBrace ( statement | inner_attribute
          |
           enclosing_doc_comment )* (
           ret=expression? |
           pat=expression_pattern )
           RightBrace;
...
if_let : If Let exp=expression Equals
            var=variable_name (
            statement | block );

for_loop : ( For var=variable_name In
              range=(
              RangeExclusive |
              RangeInclusive ) (
...
assignee_props : borrow=Ampersand?
                    mutable=Mut?;

variable_pattern : LeftParen (
                      variable_spec (
                      Comma
                      variable_spec )*
                      Comma?)
...
variable_spec : ( name=variable_name (
                   Colon type=type_spec )?)
              | anon=Underscore;
...
assignment_expression : variable_expression assignment_operator
                           expression;

variable_expression : borrow=Ampersand?
                         deref=Asterisk?
                         qualifier=path_head? variable_reference (
                         LeftBracket
                         index=expression
                         RightBracket )?;

variable_reference : name=variable_name (
                        Dot
                        variable_name )*;

variable_name : Ident;

array_literal : ( LeftBracket
                   RightBracket ) #EmptyArray
              | ( LeftBracket StringLiteral (
                   Comma
                   StringLiteral )*
              ...
              | ( LeftBracket float_literal (
                   Comma
                   float_literal )*
                   RightBracket ) #FloatArray
              | ( LeftBracket BooleanLiteral (
                   Comma
                   BooleanLiteral )*
...
expression : literal cast=variable_cast? #LiteralExpression
           ...
           | function_invocation
                cast=variable_cast? #FunctionInvocationExpression
           | assignment_expression
           ...
           | exp=variable_expression Dot
                index=BareIntLiteral
                cast=variable_cast? #TupleFieldExpression
           | leftSide=expression arithmetic_operator
                rightSide=expression #ArithmeticExpression
           | Minus? LeftParen
                leftSide=expression arithmetic_operator
                rightSide=expression RightParen #ParenthesizedArithmeticExpression
           | leftSide=expression shift_operator
                rightSide=expression #ShiftExpression
           | leftSide=expression comparison_operator
                rightSide=expression #BooleanExpression
           | Bang? LeftParen ls=expression comparison_operator
                rs=expression RightParen #ParentheizedBooleanExpression;

unsafe_expression : Unsafe LeftBrace expression
                       RightBrace;
...
function_invocation : ( qualifier=path_head?
                         func=Ident LeftParen invocation_args
                    ...
                    | (
                         qualifier=type_hint? DoubleColon
                         func=Ident LeftParen invocation_args
                         RightParen ) #TypeHintQualifiedFunctionInvocation;

type_hint : LeftAngleBracket type_spec As type_spec
               RightAngleBracket;

macro_invocation : macro=Ident Bang LeftParen invocation_args
                      RightParen;
...
tuple_expression : LeftParen expression (
                      Comma
                      expression )*
                      RightParen;
...
expression_pattern : LeftParen expression (
                        Comma
                        expression )*
                        RightParen;

variable_name_pattern : LeftParen ( (
                           Underscore
                          |
                           variable_name ) (
                           Comma (
...
int_literal : ( neg=Minus? ? value=(
                 BareIntLiteral
                |
                 FullIntLiteral )
                 type=signed_int_subtype?
...
function_spec : doc=doc_comment?
                   attrs=outer_attribute*
                   vis=visibility? Fn
                   name=Ident
                   lifetimes=lifetime_spec? LeftParen
                   params=parameter_list? RightParen
                   return_type?;

extern_block : attrs=outer_attribute* Extern LeftBrace (
...
extern_function_statement :
                               inner_attribute*
                               attrs=outer_attribute* function_spec
                               Semicolon;

closure : ( attrs=outer_attribute*
             lifetimes=lifetime_spec?
             mv=Move? Pipe
             params=parameter_list? Pipe
             return_type? (
...
struct : ( doc=doc_comment?
            outer_attribute* Struct
            name=Ident lifetimes=lifetime_spec? LeftBrace (
            struct_item ( Comma
            struct_item )*
            Comma?)? RightBrace ) #PlainStruct// XXX could split this out and include it in statement_body - checking for Semicolon here may cause problems
       | ( doc=doc_comment?
            outer_attribute* Struct
            name=Ident lifetimes=lifetime_spec? LeftParen (
            type_spec ( Comma
...
struct_instantiation : type=type_spec LeftBrace (
...
struct_instantiation_item : ( name=Ident Colon params=param_props
                               expression ) #ExplictStructItem
                          | (
                               DotDot
                               name=Ident ) #CopyStructItem;

lifetime_spec : LeftAngleBracket
                   first=lifetime (
                   Comma more=lifetime )*
                   RightAngleBracket;
...
visibility_restriction : LeftParen
                            restrictedTo=Crate RightParen #CrateVisibility
                       | LeftParen
                            restrictedTo=Super RightParen #SuperVisibility
                       | LeftParen In
...
parameter_spec : props=param_props
                    name=variable_name (
                    Colon
                    Ampersand?
                    life=lifetime?
...
param_props : ref=Ampersand?
                 life=lifetime?
                 mutable=Mut?
                 dynamic=Dyn?;

return_type : Arrow ( type=type_spec
                | typePattern=type_pattern );

type_pattern : LeftParen type_spec (
                  Comma
                  type_spec )
                  RightParen;

type_spec : path=path_head?
               type=Ident {referencedTypes.add(_ctx.getText());
}
#Named
          | type=intrinsic_type #Intrinsic
          | LeftParen RightParen #Unit;

extern_import : Extern Crate
                   create=Ident;
...
inner_attribute : InnerAttrPrefix metaitem
                     RightBracket;

outer_attribute : AttrPrefix metaitem
                     RightBracket;
...
BlockComment : BlockCommentPrefix ( ~ [*/]
                 | Slash* BlockComment
                 | Slash+ ( ~[*/]
                  ) | Asterisk+ ~[*/])*
...
RangeExclusive : DEC_DIGITS DotDot
                    DEC_DIGITS;

MatchRangeInclusive : DEC_DIGITS TripleDot
                         DEC_DIGITS;

RangeInclusive : DEC_DIGITS DotDotEquals
                    DEC_DIGITS;
...
CharLiteral : SingleQuote (
                 CHAR | Quote )
                 SingleQuote;

StringLiteral : Quote
                   STRING_ELEMENT* Quote
              | 'r'
                   RAW_STRING_BODY;

ByteLiteral : ByteLiteralPrefix (
                 BYTE | Quote )
                 SingleQuote;

ByteStringLiteral : ByteStringPrefix
                       BYTE_STRING_ELEMENT* Quote
                  | 'br'
...
FullIntLiteral : DEC_DIGITS
 ...
 //
FloatLiteral : DEC_DIGITS Dot [0-9] [0-9_]*
                  EXPONENT?
...
fragment RAW_STRING_BODY : Quote
                              RAW_CHAR*? Quote
                         | Hash RAW_STRING_BODY
                              Hash;
