...
items
    : ( use_statement | extern_import_statement | extern_block | inner_attribute | unsafe_block | struct | function | mod )*;

mod
    : doc=doc_comment? outer_attribute* Pub? Mod name=Ident ( LeftBrace ( mod | use_statement | function )* RightBrace )?;
...
block
    : LeftBrace ( statement | inner_attribute | enclosing_doc_comment )* ( ret=expression? | pat=expression_pattern ) RightBrace;
...
if_let
    : If Let exp=expression Equals var=variable_name ( statement | block );

for_loop
    : ( For var=variable_name In range=( RangeExclusive | RangeInclusive ) ( statement | block )) #ForRanged
...
variable_cast
    : As ( RawPointerMutable | RawPointerConst ) type_spec;

assignment_expression
    : variable_expression assignment_operator expression;

variable_expression
    : borrow=Ampersand? deref=Asterisk? qualifier=path_head? variable_reference ( LeftBracket index=expression RightBracket )?;
...
default_match_case
    : Comma? Underscore FatArrow ( statement_body | expression | block );
...
variable_name_pattern
    : LeftParen ( ( Underscore | variable_name ) ( Comma ( Underscore | variable_name ))*)? RightParen;
...
int_literal
    : ( neg=Minus? ? value=( BareIntLiteral | FullIntLiteral ) type=signed_int_subtype? cast=int_cast?) #SignedIntLiteral
...
int_cast
    : As ( signed_int_subtype | unsigned_int_subtype );
...
function_spec
    : doc=doc_comment? attrs=outer_attribute* vis=visibility? Fn name=Ident lifetimes=lifetime_spec? LeftParen params=parameter_list? RightParen return_type?;

extern_block
    : attrs=outer_attribute* Extern LeftBrace ( extern_function_statement ( Comma extern_function_statement )*)?;

extern_function_statement
    : inner_attribute* attrs=outer_attribute* function_spec Semicolon;

closure
    : ( attrs=outer_attribute* lifetimes=lifetime_spec? mv=Move? Pipe params=parameter_list? Pipe return_type? ( body=block | expr=expression ))
...
struct_instantiation
    : type=type_spec LeftBrace ( struct_instantiation_item ( Comma struct_instantiation_item )* Comma?)? RightBrace;
...
return_type
    : Arrow ( type=type_spec | typePattern=type_pattern );

type_pattern
    : LeftParen type_spec ( Comma type_spec ) RightParen;

type_spec
    : path=path_head? type=Ident {referencedTypes.add(_ctx.getText());
}
#Named
...
BlockComment
    : BlockCommentPrefix ( ~ [*/] | Slash* BlockComment | Slash+ ( ~[*/] ) | Asterisk+ ~[*/])* Asterisk+ Slash -> channel ( 2 );

Lifetime
    : SingleQuote ( IDENT | Static );
...
CharLiteral
    : SingleQuote ( CHAR | Quote ) SingleQuote;
...
ByteLiteral
    : ByteLiteralPrefix ( BYTE | Quote ) SingleQuote;
