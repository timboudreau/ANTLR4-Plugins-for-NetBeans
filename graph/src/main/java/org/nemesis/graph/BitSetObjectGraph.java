...
class BitSetObjectGraph<T> implements ObjectGraph<T> {
    ...
    private final Indexed<? extends T> indexed;

    BitSetObjectGraph(IntGraph graph, T[] sortedArray) {
        this(graph, new IndexedImpl<>(sortedArray));
    }

    BitSetObjectGraph(IntGraph graph, Indexed<? extends T> indexedImpl) {
        this.graph = graph;
    ...
    static class IndexedImpl<T> implements Indexed<T> {
    ...
    static final class FIndexed<T> implements Indexed<T> {
